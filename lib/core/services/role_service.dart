...
class RoleService {
  final String baseUrl = "http://techstile.sandbox.pk/api/roles"; // Emulator ke liye localhost IP
  ...
  Future<Map<String, dynamic>> addRole(String name) async {
    ...
    return {
      'success': false,
      'message': data['errors']?['name']?[0] ?? 'Role already exists',
    };
  }
...
Future<List<dynamic>> getAllPermissions() async {
  final response = await http.get(Uri.parse('http://techstile.sandbox.pk/api/permissions/all'),
  headers: AuthService.authHeaders,);
  return json.decode(response.body)['data'];
}
...
Future<List<dynamic>> getRolePermissions(int roleId) async {
  final response = await http.get(Uri.parse('http://techstile.sandbox.pk/api/role-permissions/$roleId'),
  headers: AuthService.authHeaders,);
  return json.decode(response.body)['data'];
}
