...
class RoleService {
  final String baseUrl = "http://textile2.sandbox.pk/api/roles";
  ...
  Future<List<dynamic>> getAllPermissions() async {
    final response = await http.get(
      Uri.parse('http://textile2.sandbox.pk/api/permissions/all'),
      headers: AuthService.authHeaders,
    );
    return json.decode(response.body)['data'];
  }
  ...
  Future<List<dynamic>> getRolePermissions(int roleId) async {
    final response = await http.get(
      Uri.parse('http://textile2.sandbox.pk/api/role-permissions/$roleId'),
      headers: AuthService.authHeaders,
    );
    return json.decode(response.body)['data'];
  }
  ...
  Future<bool> syncPermissions(int roleId, List<int> permIds) async {
    final response = await http.post(
      Uri.parse('http://textile2.sandbox.pk/api/permissions/sync'),
      headers: AuthService.authHeaders,
